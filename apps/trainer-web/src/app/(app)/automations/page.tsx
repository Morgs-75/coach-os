  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="card p-6">
          <p className="text-sm font-medium text-gray-500">Runs Today</p>
          <p className="mt-2 text-3xl font-semibold text-green-600">
            {runStats.ok ?? 0}
          </p>
        </div>
        <div className="card p-6">
          <p className="text-sm font-medium text-gray-500">Skipped (Guardrails)</p>
              ...
              recentRuns.map((run: any) => (
                <div key={run.id} className="px-6 py-4">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-400">{formatDate(run.fired_at)}</span>
                  </div>
                  {run.reason && (
                    <p className="text-xs text-gray-400 mt-1">{run.reason}</p>
                  )}
                </div>
              ))
            ) : (
              <p className="px-6 py-8 text-center text-gray-500">No runs yet</p>
            )}
          </div>
        </div>
      </div>
      ...
      {/* Template Gallery */}
      <div className="mt-8">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Automation Templates</h2>
